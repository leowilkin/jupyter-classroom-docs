...
<Aside type="tip" title="`tljh-config set` syntax">
Pass values as positional arguments — no `=`, no shell-quoted values around plain strings:

```bash
# wrong — produces "unrecognized arguments: email"
sudo tljh-config set auth.AzureAdOAuthenticator.username_claim = "email"

# right
sudo tljh-config set auth.AzureAdOAuthenticator.username_claim email
```

Quotes are only needed when a value contains spaces or shell metacharacters; for normal claim names, paths and booleans, leave them off.
</Aside>
...
<CardGrid>
    ...
    <LinkCard title="Auto-cleaning runaway notebooks" href="/configuration/auto-cleaner/" />
    <LinkCard title="Serving over HTTPS on a domain" href="/configuration/https/" />
</CardGrid>